    ...
    fig = (
        X_train.assign(Outcome=y_train.astype(str).replace({'0': 'no diabetes', '1': 'yes diabetes'}))
                .plot(kind='scatter', x='Glucose', y='BMI', color='Outcome', 
                      color_discrete_map={'no diabetes': 'orange', 'yes diabetes': '#3d81f6'},
...
def show_decision_boundary(model, X_train, y_train, title=''):
    ...
    # Update layout
    fig.update_layout(
        title=dict(text=title, font=dict(size=20, family=FONT_FAMILY)),
        xaxis_title='Glucose',
        yaxis_title='BMI',
        showlegend=True,
...
def show_logistic(model_logistic, X_train, y_train, t=0.5, show_training=False, show_threshold=False):
    ...
    fig.update_layout(
        title=dict(
            text=f'Predicted Probability of Diabetes Given Glucose and BMI' + (f'<br>Threshold = {t}' if t != 0.5 else ''),
            font=dict(family=FONT_FAMILY)
        ),
        width=1000, 
        height=800, 
...
def decision_boundary_slide_k(X_train, y_train, k_max=50):
    ...
    # Update layout
    fig.update_layout(
        title=dict(text='k-NN Decision Boundary for k = 1', font=dict(size=20, family=FONT_FAMILY)),
        xaxis_title='Glucose',
        yaxis_title='BMI',
        showlegend=True,
        legend=dict(font=dict(size=12, family=FONT_FAMILY)),
...
def penguin_scatter_2d(X_train, y_train):
    ...
    # Apply consistent formatting before adding image
    fig.update_layout(
        paper_bgcolor='white',
        plot_bgcolor='white',
        font=dict(family=FONT_FAMILY),
    ...
    fig.add_layout_image(
        dict(
            source="01-intro-imgs/lter_penguins.png",
            xref="paper",
            yref="paper",
            x=0.05,
            y=0.95,
